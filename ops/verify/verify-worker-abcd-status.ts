 ...
 * 워커 AB=CD 상태 회귀 검증
 *
 * 고정 4h 픽스처를 실제 AbcdEngine.feed()에 순서대로 넣고,
 * 라이브 모니터링 탭이 읽는 getStatus().trackersList 상태를 baseline과 비교한다.
 *
 * 실행:   node --experimental-strip-types ops/verify/verify-worker-abcd-status.ts
 * 갱신:   node --experimental-strip-types ops/verify/verify-worker-abcd-status.ts --update
 *
 * 범위: AB=CD 워커 엔진의 상태머신(scanning/signal/active/done) 재현 검증.
...
import { AbcdEngine } from '../../labs/trading/worker/src/lib/abcd-engine.ts';
import type { Candle } from '../../labs/trading/worker/src/lib/candle-feed.ts';

const HERE = dirname(fileURLToPath(import.meta.url));
const FIXTURE_DIR = join(HERE, 'fixtures');
const BASELINE_PATH = join(FIXTURE_DIR, 'expected-worker-abcd-status.json');
...
if (process.argv.includes('--update')) {
  ...
  console.log('✅ worker abcd baseline 갱신:', BASELINE_PATH);
  ...
  try {
    ...
    console.error('❌ worker abcd baseline 없음 — 먼저 --update로 생성하세요.');
    process.exit(1);
  }
  ...
  if (missing.length || added.length) {
    console.error(`❌ worker abcd 상태 불일치: 기준 ${expected.length}개 vs 현재 ${current.length}개`);
    ...
    console.error('\n의도된 워커 상태머신 변경이면: node --experimental-strip-types ops/verify/verify-worker-abcd-status.ts --update');
    process.exit(1);
  }

  console.log('✅ worker abcd 상태 동일성 통과 —', JSON.stringify(actual.summary));
}
