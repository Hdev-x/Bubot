...
```text
apps/
  api/         Spring Boot 3.5 · MyBatis · PostgreSQL — 인증(JWT), 시세 프록시·WebSocket 중계, 거래소 계좌 조회
  web/         React 19 · TypeScript · Vite — Desktop(/web)·Mobile(/mobile) 두 진입점, Vitest
shared/        하모닉·SMC·파동·피벗 순수 계산 엔진. web과 worker가 함께 사용 (외부 의존 없음)
labs/
  trading/
    worker/    봇 매매·모니터링 워커 (Node, Beta 비활성)
    web/       Beta에서 분리한 자동매매·Paper·Backtest UI 보존본 (실행 진입점 없음)
ops/           기동·배포 스크립트, ops/verify 회귀 검사와 baseline fixtures
docs/          목적·명령·Git 규칙·문서 운영 정본
work-status/   현재 상태·결정·미결·Roadmap·Work Package PLAN
```

의존 방향은 `apps/web → shared`, `labs → shared`, `labs/trading/web → apps/web`(alias)이며 `apps`는 `labs`를 import하지 않는다.
