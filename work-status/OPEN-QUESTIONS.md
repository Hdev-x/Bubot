...
- 질문: OQ-20260905-04 kline 중계의 구독 상태 전이는 PR #64에서 `refLock`으로 원자화됐지만, SUBSCRIBE/UNSUBSCRIBE 전송이 lock 밖·스케줄러 스레드에서 나가 상태 순서와 전송 순서가 어긋날 수 있다(유예 만료 UNSUBSCRIBE가 새 SUBSCRIBE 뒤에 도착). 연결별 전송 큐로 직렬화할지 | 2차·4차 리뷰, `BinanceKlineRelayService.Conn`
