...
- 프로젝트가 지금 달성하려는 결과: 다음 WP 선택 대기. 후보는 큰 파일 분해 나머지(`MarketChart.tsx` 1,585·`useAutoPatterns.ts` 990·`OrderPage.tsx` 910, `DrawingToolbar.tsx` 641), 차트 훅 정리(T-04f `useLivePrice`), API 중계 재연결 보강(Fast Path), Beta 배포 준비(T-05).

## TODO

> 우선순위 순이다. 각 항목은 반드시 한 줄로 쓰고 완료하면 지운다.

1. 다음 WP 결정 후 PLAN 작성
...
- API의 거래소 중계(`CoinRealtimeWebSocketService`·`BinanceKlineRelayService`·Binance 티커 WS)는 끊긴 뒤 재연결에 실패하면 다시 시도하지 않는 약점이 있다(2026-09-04 로그: 포트 고갈·reconnecting 플래그 고착). Mobile은 모든 실시간 데이터가 이 중계에 의존해 통째로 멈춘다. 임시 조치는 API 재시작, 근본 수정은 재연결 백오프·상태 초기화(Fast Path 후보).
