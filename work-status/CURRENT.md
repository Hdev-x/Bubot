...
- 프로젝트가 지금 달성하려는 결과: 배포(T-05)는 나중 문제로 보류(사용자, 2026-09-05). 리팩터링 전체 최종 리뷰와 브리핑 문서 작성. 다음 WP 순서는 사용자 결정(2026-09-05): T-04f → T-05 배포 → OQ-11 lint 경고 축소. 배포 전에 배포 대상·도메인·DB 위치(OQ-20260903-04) 결정 필요. 후보는 T-04f `useLivePrice` 분리, OQ-11 lint 경고 축소, OQ-04 Beta 배포.

## TODO

> 우선순위 순이다. 각 항목은 반드시 한 줄로 쓰고 완료하면 지운다.

1. T-05 준비: 배포 대상·도메인·DB 위치 결정(OQ-20260903-04) → PLAN 작성
