...
- job: `web`(`apps/web` — `npm ci`, `npm run lint`, `npm test`, `npm run build`, `npm run build:web`), `api`(`apps/api` — `./gradlew compileJava`).
...
- 아직 CI에 없는 것: API `test`(DB·env 의존 제거 후 추가), Worker verify(baseline drift 정리 후 추가). Web lint는 2026-09-03부터 포함(error 0 유지, warning은 baseline).
